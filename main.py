...
# Configurar Matplotlib para trabajar en segundo plano (Evita conflictos con la GUI)
import matplotlib
matplotlib.use('Agg')
...
def procesar_asistencia(ruta_archivo, txt_output, horas_config):
    try:
        ...
        txt_output.insert(tk.END, f"   -> ¡Archivo cargado! Total registros: {len(df)}\n")
        ...
        txt_output.insert(tk.END, "3. Convirtiendo texto a fechas reales...\n")
        ...
        txt_output.insert(tk.END, "4. Aplicando los horarios configurados para el análisis...\n")
        ...
        for (empleado, fecha), grupo in df.groupby(['Nombre', 'Fecha']):
            # Convertir todas las marcas del empleado en ese día a minutos y ordenarlas
        ...
        # Renombrar columnas dinámicamente en el reporte visual
        ...
        # Imprimir en consola de la interfaz
        ...
        messagebox.showinfo("¡Completado!", "El reporte dinámico se procesó exitosamente.")

    except Exception as e:
        messagebox.showerror("Error de Formato", f"Asegúrate de escribir las horas en formato HH:MM:SS (Ej: 06:30:00).\nDetalle: {str(e)}")
...
# Grid interna para acomodar las cajas de texto de los horarios
